...
typedef map <Label, MemPos>											UseTable;
typedef map <Label, MemPos>											DefTable;
...
Labels getLabels (CodeLines* codeLines, Instructions instructions, Directives directives) {
    ...
    for (CodeLines::iterator it = codeLines->begin(); it != codeLines->end(); ++it) {
        ...
        if (":" == it->second.front().substr(it->second.front().length() - 1, 1)) {
            ...
            if("EXTERN" == it->second.front()) {
            	labels[tempLabel] = 0;
            	useTable[tempLabel] = memPos;
            }
        }
    ...
    for (CodeLines::iterator codeLine = codeLines.begin(); codeLine != codeLines.end(); ++codeLine) {
            ...
            for (Tokens::iterator token = codeLine->second.begin() + 1; token != codeLine->second.end(); ++token) {
                ...
                *token = token->substr(0,token->find("+"));
                ...
                if (labels.find(*token) != labels.end()) {
                    ...
                    if(useTable.find(*token) != useTable.end())
                        useTable[*token] = memPos;
        ...
        // Achou uma Diretiva
            ...
            if ("SPACE" == codeLine->second.front()) {
                ...
                output.append(" ");
                tempSS.str("");
            } else if ("PUBLIC" == codeLine->second.front()) {
                defTable[codeLine->second[1]] = labels[codeLine->second[1]];
            }
...
int main(int argc, char const *argv[]) {
    ...
    if (errors.empty()) {
        ...
        if(!defTable.empty()) {
            flagModular = 1;
            output.append("TABLE DEFINITION");
            output.append("\n");
            for (DefTable::iterator i = defTable.begin(); i != defTable.end(); ++i) {
                output.append(i->first);
                output.append(" ");
                stringstream ss;
                ss << i->second;
